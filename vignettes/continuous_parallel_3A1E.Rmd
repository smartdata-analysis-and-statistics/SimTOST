...
* The `list_y_comparator` argument determines the endpoint set, so the
  multiplicity calculation is based on the endpoints actually included in
  each comparison.
* Comparisons are evaluated jointly when both reference products are included.
  Use `type1Error()` to check the resulting trial-level error rate when the
  design contains several comparison families.
...
We then update the one-comparator result with the second comparison. This
avoids repeating the common treatment-arm, distribution, and simulation
arguments while recalculating the joint design.
