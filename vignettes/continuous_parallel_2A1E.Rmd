...
### Scenario
Drug B is a well-established biologic drug used to control blood pressure. Its exclusive marketing license has expired, creating an opportunity for other companies to develop biosimilars. Drug A is a new competing drug being developed as a potential biosimilar to Drug B. The goal is to determine whether Drug A meets the FDA's biosimilarity requirements in terms of safety, purity, and therapeutic response when compared to Drug B.
...
* Mean Difference: As per the FDA guidelines, the assumed difference between the two groups is set to $\delta = \sigma/8 = 2.25$ mmHg.
...
(N_ss <- sampleSize(
  ...
  distribution = "norm",               # Assumes normal distribution
...
To account for an anticipated dropout rate of 20% in each group, we update the
existing calculation. The update retains the endpoint assumptions and changes
the dropout and optimization settings only; the faster routine still searches
for the smallest integer sample size that meets or exceeds the target power.

```{r}
# Adjusted sample size calculation with 20% dropout rate
(N_ss_dropout <- update(
  N_ss,
  dropout = c("R" = 0.20, "T" = 0.20),
