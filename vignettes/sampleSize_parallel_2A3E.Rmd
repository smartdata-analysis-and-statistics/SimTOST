...
(N_mp_bon <- sampleSize(
  ...
  k = c("EMA" = 1),          # Reject at least one null hypothesis
...
(N_mp_k <- sampleSize(
  ...
  vareq = TRUE,              # Assume equal variances
  lognorm = TRUE,            # Log-normal distribution assumption
  k = c("EMA" = 2),          # Reject at least one null hypothesis
...
* Equivalence for at least one secondary endpoint if the primary requirement is met.

```{r}
(N_mp_seq1 <- sampleSize(
  power           = 0.9,                              # Target power
  alpha           = 0.05,                             # Significance level
  mu_list         = mu_list,                          # List of means
  sigma_list      = sigma_list,                       # List of standard deviations
...
Recall we needed `r sim_AUCinf$response$n_total` patients to demonstrate equivalence for AUCinf. We now need `r N_mp_seq1$response$n_total` patients to demonstrate equivalence for AUCinf, and if equivalence holds, to also demonstrate equivalence for AUClast and Cmax.

Note that if we change the primary endpoint to AUClast, we have:

```{r}
(N_mp_seq2 <- sampleSize(
  power           = 0.9,                              # Target power
  alpha           = 0.05,                             # Significance level
  mu_list         = mu_list,                          # List of means
  sigma_list      = sigma_list,                       # List of standard deviations
  ...
  k               = c("EMA" = 1),                     # Reject at least one null hypothesis
  adjust          = "seq",                            # Sequential adjustment method
  type_y          = c("AUCinf" = 2, "AUClast" = 1, "Cmax" = 2) , # Endpoint types
  ncores          = 1,                                # Number of cores for computation
  nsim            = 1000,                             # Number of stochastic simulations
  seed            = 1234                              # Random seed for reproducibility
))
...
The required sample size is now similar to the independent test for AUClast, which is `r sim_AUClast$response$n_total`.
