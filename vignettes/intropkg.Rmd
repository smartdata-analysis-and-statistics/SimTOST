...
### Mielke's t-adjustment for k-of-m decisions
When the decision rule requires at least `k` of `m` endpoints, the null
hypothesis is a partial-conjunction hypothesis. The Bonferroni calibration for
this rule uses the `m-k+1` boundary endpoints:

$$\alpha_{t}= \frac{\alpha}{m-k+1}$$

Mielke et al. (2018) [@mielke_sample_2018] describe this as the strong
k-out-of-m adjustment for the partial-conjunction decision. It is available
in `sampleSize()`, `simPower()`, and the count-outcome functions with
`adjust = "t"`; the legacy `"pc"` label remains an alias. The effective `m`
is determined separately for each comparator from the endpoints actually
selected or inferred. The `adjust = "k"` option, in contrast, uses
`k * alpha / m` and provides weak complete-null control.
