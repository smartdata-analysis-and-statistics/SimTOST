...
that must demonstrate equivalence. For example, with `m = 3`, `k = 2`, and
`alpha = 0.05`, the endpoint-wise level is `0.025`. This calibration controls
...
The adjustment is available in `sampleSize()`, `simPower()`, and the
count-outcome functions:

```{r t-adjustment-example, eval = FALSE}
sampleSize(
  ..., k = 2, adjust = "t"
)
...
The effective `m` is determined separately for each comparator from the
endpoints actually selected or inferred. The legacy `adjust = "pc"` and
`adjust = "partial-conjunction"` labels are accepted as aliases for `"t"`.
In contrast, `adjust = "k"` uses `k * alpha / m` and provides weak
