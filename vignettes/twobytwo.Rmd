...
```{r, eval = FALSE}
#Using `PowerTOST`, the required sample size is `r N_pt["Sample size"]`. In this case, the required sample size  is smaller than the result from `SimTOST`. Our algorithm selects the sample size that achieve in average at least the target power 90\%, in this case a sample size of `r N_ss$response[["n_total"]]` with a power of  `r N_ss$response[["power"]]`. But by inspecting the iteration table, we observe that with a sample size of 22, we can obtain a target power inside the confidence interval of the calculated power.
```
...
```{r, eval = FALSE}
# Using `PowerTOST`, the required sample size is `r N_pt["Sample size"]`. In this case, the required sample size  is equal to the result from `SimTOST`.
```
